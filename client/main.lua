...
local function LoadRadarProps()
	...
	TriggerEvent('RL-speedcameras:ShowRadarBlip')
end
...
local function HandlespeedCam(kmhSpeed, maxSpeed, Plate, vehicleModel, radarStreet)
	...
	if TooMuchSpeed >= 25 then
		TriggerServerEvent('RL-speedcameras:PayFine', GetPlayerServerId(PlayerId()), Plate, kmhSpeed, maxSpeed, fine, vehicleModel, radarStreet, displaymph)
	end
end
...
RegisterNetEvent('RL-speedcameras:ShowRadarBlip', function()
	coyottestate = true
...
RegisterNetEvent('RL-speedcameras:ShowRadarProp', function()
	LoadRadarProps()
end)

RegisterNetEvent('RL-speedcameras:RemoveRadarBlip', function()
	coyottestate = false
	for i=1, #Blips, 1 do
		RemoveBlip(Blips[i])
		Blips[i] = nil
